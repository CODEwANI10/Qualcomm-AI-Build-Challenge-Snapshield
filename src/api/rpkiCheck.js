...
async function _fetchRPKI(asn, prefix, key) {
  ...
  if (!/^\d+$/.test(asnNum)) {
    console.warn(`[SnapShield RPKI] Skipping invalid ASN: ${asn}`)
    cacheResult(key, null, FAIL_CACHE_MS)
    return null
  }
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[SnapShield RPKI] ${prefix}: HTTP ${res.status} — skipping RPKI check`)
      cacheResult(key, null, FAIL_CACHE_MS)
      return null
    }
    ...
    if (err.name !== 'AbortError') {
      console.warn(`[SnapShield RPKI] ${prefix}: network error — ${err.message}`)
    }
    cacheResult(key, null, FAIL_CACHE_MS)
    return null
  }
