...
export default function WhatHappened() {
  ...
  const isReal     = latest.isRealData && !latest.isSimulated
  const cmReady    = latest.countermeasuresReady
  ...
  return (
    <div style={baseStyle}>
      ...
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text-primary)', lineHeight: 1.7 }}>
        ...
        {isReal
          ? cmReady
            ? <span style={{ color: '#ffd60a', fontWeight: 700 }}>SnapShield generated real RPKI/RTBH/Flowspec countermeasures — pending NOC authorization.</span>
            : <span style={{ color: '#ffd60a' }}>SnapShield detected it in under 90 seconds. Generating countermeasures…</span>
          : mitigated
          ? <span style={{ color: '#30d158', fontWeight: 700 }}>[DEMO] Simulated mitigation complete.</span>
          : <span style={{ color: '#ffd60a' }}>[DEMO] SnapShield detected it in under 90 seconds. Response in progress.</span>
        }
      </div>
    </div>
  )
