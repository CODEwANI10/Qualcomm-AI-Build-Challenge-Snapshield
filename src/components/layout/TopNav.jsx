...
export default function TopNav({ onBreachClick }) {
  const systemTime  = useSnapShieldStore(s => s.systemTime)
  const risStatus   = useSnapShieldStore(s => s.risStatus)
  const appMode     = useSnapShieldStore(s => s.appMode)
  const setAppMode  = useSnapShieldStore(s => s.setAppMode)
  const notifications = useSnapShieldStore(s => s.notifications)
  ...
  function handleSimulateInterruption() {
    const ok = window.confirm(
      'Simulate Interruption?\n\nThis force-kills the current session (like a crash or dropped tab) and reloads the page. On reload, SnapShield will detect the unfinished work and recover it — watch for the recovery banner.'
    )
    if (!ok) return
    window.location.reload()
  }
  const [showChangeLog, setShowChangeLog] = useState(false)
  const changeHistory = useSnapShieldStore(s => s.changeHistory)
  const isPaused       = useSnapShieldStore(s => s.isPaused)
  const pauseSession   = useSnapShieldStore(s => s.pauseSession)
  const resumeSession  = useSnapShieldStore(s => s.resumeSession)
  ...
  // Live threat level from on-device ONNX classifier (0-100)
  const [threatLevel, setThreatLevel] = useState(0)
  useEffect(() => {
    const handler = (e) => {
      try {
        const d = JSON.parse(e.detail ?? '{}')
        // Slide 5: Score S ∈ [0.0, 1.0] → SeverityMeter level 0-100
        const score = d.score ?? 0
        if (score >= 0.85) {
          setThreatLevel(Math.round(score * 100))
        } else {
          // Decay toward 0 for normal traffic
          setThreatLevel(l => Math.max(0, l - 8))
        }
      } catch {}
    }
    ...
    <>
      <nav style={{
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            <div style={{ fontFamily:'var(--font-display)', fontSize:20, fontWeight:900, color:'#00ff88', letterSpacing:'-0.02em', lineHeight:1 }}>SnapShield</div>
          ...
          {/* Pause / Resume — freezes the entire incident-processing pipeline
              ...
              via the persisted store — see useSnapShieldStore.js. */}
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          {/* NPU status badge — polls /api/health every 10s */}
          <NPUStatusBadge />

          <div style={{ width:1, height:24, background:'var(--border-subtle)' }} />

          {/* SeverityMeter compact pill */}
          <SeverityMeter level={threatLevel} compact={true} />
