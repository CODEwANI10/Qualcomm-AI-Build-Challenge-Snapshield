...
import { querySnapShield } from '../../api/snapshieldAPI.js'

const SYSTEM_PROMPT = `You are SnapShield, an on-device network security analyst powered by Llama 3.2 3B on the Qualcomm Hexagon NPU. Answer questions about BGP incidents, network threats, and Indian internet infrastructure concisely and technically. Keep answers under 100 words. No markdown formatting.`
...
async function queryAI(question, context) {
  ...
  try {
    ...
    const result = await querySnapShield(
      SYSTEM_PROMPT,
      `Current incidents:\n${context}\n\nQuestion: ${question}`,
      200,
      0.3,
...
export default function ConversationalQuery({ compact, incidentId }) {
  ...
  return (
    <div style={{ marginTop:16, background:'rgba(255,255,255,0.02)', border:'1px solid var(--border-subtle)', borderRadius:4, padding:12 }}>
      <div style={{ fontFamily:'var(--font-mono)', fontSize:8, color:'var(--accent-green)', letterSpacing:'0.15em', marginBottom:10 }}>◈ ASK SNAPSHIELD AI</div>
